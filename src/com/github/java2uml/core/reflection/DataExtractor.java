...
public class DataExtractor {
     ...
     * @return boolean
     */
    public static boolean extract(final Set<Class> classes) {
        ...
        for (Class clazz : classes) {
            ...
            if ((Boolean)Options.get(Options.OPTION_LOLLITOP)) {
            ...
            for (Method method : methods) {
                ...
                for (Class exception : method.getExceptionTypes()) {
                	if (classes.contains(exception)) {
                		...
                		if ((Boolean)Options.get(Options.OPTION_ASSOCIATION)) {
        ...
        for (Class clazz : classes) {
            ...
            // объявление связей...
            if ((Boolean)Options.get(Options.OPTION_EXTENTION)) {
            ...
            if ((Boolean)Options.get(Options.OPTION_IMPLEMENTATION)) {
            ...
            for (Field fieldClass : fieldClasses) {
            	if (fieldClass.getType() instanceof Object) {
                    if (classes.contains(fieldClass.getType())) {
                    	...
                    	if ((Boolean)Options.get(Options.OPTION_AGGREGATION)) {
                    		source.append(className);
                            source.append(" o-- ");
                	...
                	if (fieldClass.getType().isArray()) {
            			...
            			if (names.contains(arrayClassName) && !arrayClassName.equals(className)) {
            				...
            				if ((Boolean)Options.get(Options.OPTION_COMPOSITION)) {
            					source.append(className);
                                source.append(" *-- ");
...
//            		}
     ...
     * @return boolean
     */
    private static boolean save(final String source) {
